...
## 2 基本术语
* 私钥  
  私钥是2的256次方的一个随机数。重要的事情说三遍：私钥即一切！私钥即一切！！私钥即一切！！！
* 助记词  
...
## 10 常见问题&解决方案
### 10.1 重复上账
资产服务接收到充币消息时，一定要做好业务幂等校验，交易hash和index保证唯一，有些智能可以将多笔交易
打包在一个TX中，因此需要通过数组index来区分，数据库也要设计关联唯一索引。
### 10.2 重复提币
提币服务接收到资产服务的提币请求时，首先是业务反查，避免人为伪造消息推送，其次还是要做好业务幂等校验，同一笔
提币订单在处理完成时（成功或者失败），不应该执行第二次签名广播。
### 10.3 提币失败
特别针对使用nonce值管理交易顺序的如ETH，APTOS，在提币的时候，同一个出款地址，需要保证
nonce是顺序无遗漏的，所以需要自己管理，因为如果是从链上获取最新的nonce，在一笔交易未被
确认前，返回的nonce都一样，如果使用同样的nonce去签名，注定会失败，因此需要保证
nonce的正确性； 总之同一个出款地址，最好能做到串行签名和广播，避免多笔交易冲突。
对外部的请求来说，一旦满足了提币条件，最后得到的结果一定是成功，有问题必须是钱包系统
自己解决。
### 10.4 分布式锁与数据库事务使用问题
处理不同的链提币，使用不同的锁是没问题的，但要注意锁过期时间，与数据库
的事务处理，有一种情况是，锁的过期时间过了，但事务还没提交，这时会导致下一个
拿到了锁的提币业务出现意外，以为上一次业务处理还没有提交最终的结果，
本次业务拿到和上一次业务一样的数据处理，严重时可能导致重复出款，需要特别注意。
## 代码规范
...
## 项目管理
兵马未动粮草先行，对于我们今天的开发团队来说，就是兵马未动，文档先行。尤其是后疫情时代，我们进入到了
一个混合办工时代，集中与远程办公结合，除了称手的IM工具，文档是推进业务的主要记录。
需求文档，需要分析与详细设计，里程碑计划，工作计划，日报，周报等，都需要有文档记录。
## 开发管理
一般流程：需求分析-》详设-》评审-》开发-》dev-》sit-》uat-》prod。一定要对数据有敬畏之心，
不仅仅是prod环境，测试环境，uat环境的问题，不要通过改数据通过，一定要找到异常数据的原因，
代码不骗人，所以不要自欺欺人。
